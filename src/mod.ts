...
function normalizePattern(
  ...
  if (parentDirectoryMatch?.[0]) {
    ...
    props.root =
      newCommonPath.length > 0 ? path.posix.join(cwd, ...newCommonPath) : cwd;
  }
...
function crawl(options: GlobOptions, cwd: string, sync: boolean) {
  ...
  const matcher = rematch(processed.match, {
    dot: options.dot,
    nocase,
    ignore: processed.ignore,
  });

  const ignore = rematch(processed.ignore, {
    dot: options.dot,
    nocase,
  });
    ...
    filters: [
      options.debug
        ? (p: string, isDirectory: boolean): boolean => {
            ...
            const matches = matcher(path) as boolean | { isMatch: boolean };
            if (matches) {
              log(`matched ${path}`);
            }
            return typeof matches === "object" ? matches.isMatch : !!matches;
          }
        : (p: string, isDirectory: boolean): boolean => {
            const result = matcher(
              processPath(p, cwd, props.root, isDirectory, options.absolute),
            ) as boolean | { isMatch: boolean };
            return typeof result === "object" ? result.isMatch : !!result;
          },
    ],
    exclude: options.debug
      ? (_: string, p: string): boolean => {
          const relativePath = processPath(p, cwd, props.root, true, true);
          const skipped =
            (relativePath !== "." && !partialMatcher(relativePath)) ||
            (ignore(relativePath) as boolean | { isMatch: boolean });
          if (skipped) {
            log(`skipped ${p}`);
          } else {
            log(`crawling ${p}`);
          }
          return typeof skipped === "object" ? skipped.isMatch : !!skipped;
        }
      : (_: string, p: string): boolean => {
          const relativePath = processPath(p, cwd, props.root, true, true);
          const result =
...
export async function glob(
  ...
  const opts =
    Array.isArray(patternsOrOptions) || typeof patternsOrOptions === "string"
      ...
      : patternsOrOptions;
  const cwd = opts.cwd
    ? path.resolve(opts.cwd).replace(BACKSLASHES, "/")
...
export { convertPathToPattern, escapePath, isDynamicPattern } from "./utils.js";
