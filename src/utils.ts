...
export function getPartialMatcher(
  patterns: string[],
  options?: PartialMatcherOptions,
): PartialMatcher {
  ...
  for (let i = 0; i < patternsCount; i++) {
    ...
    for (let j = 0; j < partsCount; j++) {
      partRegexes[j] = rematch.makeRe(
        parts[j],
        options,
      ); /* Property 'makeRe' does not exist on type '(glob: any, options: any, returnState?: boolean) => Matcher'.ts(2339)      */
    }
...
export function splitPattern(path: string): string[] {
  const result = rematch.scan(
    path,
    splitPatternOptions,
  ); /* Property 'scan' does not exist on type '(glob: any, options: any, returnState?: boolean) => Matcher'.ts(2339)  */
...
export const escapePosixPath = (path: string): string =>
  path.replace(POSIX_UNESCAPED_GLOB_SYMBOLS, "\\$&");
export const escapeWin32Path = (path: string): string =>
  path.replace(WIN32_UNESCAPED_GLOB_SYMBOLS, "\\$&");

export const escapePath: (path: string) => string = isWin
  ? escapeWin32Path
  : escapePosixPath;
// #endregion
...
export function isDynamicPattern(
  ...
  const scan = rematch.scan(pattern);
